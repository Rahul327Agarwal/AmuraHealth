...
interface IStyleProps extends IProps {
  isLoadingStatusChangePopup: boolean;
}

export const useStyles = makeStyles<IStyleProps>()(
  (
    theme,
    { isBorderRadius, isClientSelected, isEmergency, isBgWhite, isExtend, cardData, edgeColor, isLoadingStatusChangePopup }
  ) => {
    return {
      mainContainer: {
        display: 'flex',
      ...
      roleName: {
        backgroundColor: isClientSelected ? '#F1F1F1' : '#FFFFFF',
        borderRadius: '4px',
        padding: '4px 8px',
        color: '#5C5A61',
        margin: '0px',
      ...
      leftBox: {},
      gcStatusIcon: {
        opacity: isLoadingStatusChangePopup ? 0.5 : 1,
        pointerEvents: isLoadingStatusChangePopup ? 'none' : 'all',

        '& rect': {
          fill:
            cardData && getStatusOfClient(cardData)?.toLowerCase() === 'new'
      ...
      Wrapper: {
        flex: 1,
        border: `1px solid ${isClientSelected ? theme.palette.colors.gray[50] : theme.palette.colors.gray[100]}`,
        borderRadius: '36px',
        height: '26px',
      },
      dotWrapper: {
