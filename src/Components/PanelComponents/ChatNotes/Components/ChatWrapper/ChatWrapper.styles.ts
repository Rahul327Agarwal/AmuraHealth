...
export const useStyles = makeStyles<IProps>()((theme, props) => ({
  chatRowWrapper: {
    ...
    display: 'flex',
    justifyContent: props?.isOurMessage ? 'flex-end' : 'flex-start',
    paddingLeft: props?.isCommonMessage ? '16px' : '50px',
    paddingRight: '10px',
    marginBottom: '20px',
    transition: 'all .3s ease-in-out',
  },
  profileName: {
    marginLeft: props?.isCommonMessage ? '0px' : '-40px',
  },
  chatWrapper: {
    display: 'flex',
    ...
    alignItems: props?.isOurMessage ? 'flex-end' : '',
  },
  chatBubble: {
    backgroundColor: props?.bgColor || props.isOurMessage ? theme.palette.colors.gray[50] : theme.palette.colors.gray[25],
    borderRadius: props?.isCommonMessage ? '8px' : props?.isOurMessage ? '8px 0px 8px 8px ' : '0px 8px 8px 8px',
    minHeight: '52px',
    paddingTop: '20px',
    paddingRight: '16px',
    paddingBottom: '16px',
