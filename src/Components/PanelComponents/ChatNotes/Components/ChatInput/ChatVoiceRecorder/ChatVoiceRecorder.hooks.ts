...
import {
  Recorder as ChatRecorderState,
  Interval,
  AudioTrack,
  MediaRecorderEvent,
  ChatVoiceNoteMessage,
} from './ChatVoiceRecorder.types';

const initialState: ChatRecorderState = {
  recordingMinutes: 0,
  recordingSeconds: 0,
  initRecording: false,
  mediaStream: null,
...
export function useChatRecorder() {
  const [recorderState, setRecorderState] = useState<ChatRecorderState>(initialState);
  ...
  useEffect(() => {
    ...
    if (recorderState.initRecording)
      recordingInterval = setInterval(() => {
        setRecorderState((prevState: ChatRecorderState) => {
  ...
  function getVoiceNoteMessage(): ChatVoiceNoteMessage {
    return {
  ...
  return {
    recorderState,
    startRecording: () => ChatVoiceRecorderFns.startRecording(setRecorderState),
    cancelRecording: () => setRecorderState(initialState),
    saveRecording: () => ChatVoiceRecorderFns.saveRecording(recorderState.mediaRecorder),
    pauseRecording: () => ChatVoiceRecorderFns.pauseRecording(recorderState.mediaRecorder),
    resumeRecording: () => ChatVoiceRecorderFns.resumeRecording(recorderState.mediaRecorder),
    getVoiceNoteMessage,
  };
}
