...
import { PostSnippet, PostSnippetHeader } from '../../PostSnippet';
import { getVoiceNoteFromURL } from '../../RightMessage/RightMessage.functions';
import { POST_KEYS, setPostFiles, setSubPostFiles } from '../postPreview/Preview.function';
import {
  ...
  showingResponseMsgs,
} from './Preview.function';
import { MenuListItemStyled, MenuListStyled, useStyles } from './Preview.styles';
import { IProps } from './Preview.types';
import { useCommonStyles } from '../../../../../Common/Theme/CommonStyles';
import { CloseIcon, PostCollectionsIcon, PostIcon, ResponseCheck, ResponseCircleIcon } from '../../PostCollections.svgs';
...
const SubPostPreview = (props: IProps) => {
    ...
    if (Object.keys(mediaFiles).length) {
      (async () => {
        ...
        for (const key in mediaFiles) {
          const element = (mediaFiles as any)[key];
          if (element) {
  ...
  const handleInvisibleKey = async (data: { id: string; value: string }) => {
    const { id, value } = data;
    let preClone: any = { ...invisibleKeys };
    let preValue = preClone[id] || [];
  ...
  const addPostPreviewData = async () => {
    ...
    const payload = {
      ...
      invisibleKeys: (invisibleKeys as any)[VISIBLE_KEY.SC],
    ...
    await callAddSubCollectionPost(panelId, props, payload);
        ...
        return (
          <div className={classes.mainwrapper}>
            <div className={classes.attachedVideo}>
              ...
              <VideoPlayerNewDesign src={imediaURL} sessions={sessions} />
            </div>
          </div>
        );
      case 'file':
        ...
        return (
          <div className={classes.mainwrapper}>
            <div className={classes.attachedVideo}>
              ...
              <MediaAudio src={imediaURL} whiteTheme sessions={sessions} />
            </div>
          </div>
        );
    }
  ...
  return (
    <div className={classes.rootContainer}>
      <div className={`${classes.dflex} ${classes.titleWraper}`}>
        ...
        <div className={classes.iconStyle} onClick={() => setActionType('HOME')}>
          {<CloseIcon />}
        </div>
      </div>
      <div className={classes.headerWrap}>
        ...
        {/* <PostSnippet
          title="Title"
          content={subCollectionData?.topics?.title?.snippet}
          handleToggleView={() => handleInvisibleKey({ id: VISIBLE_KEY.SC, value: visibleKeys.TITLE })}
          isHideView={invisibleKeys[VISIBLE_KEY.SC]?.includes(visibleKeys.TITLE)}
        /> */}
        ...
        <PostSnippet
          ...
          isHideView={(invisibleKeys as any)[VISIBLE_KEY.SC]?.includes(visibleKeys.DESCRIPTON)}
        />
        <PostSnippet
          ...
          isHideView={(invisibleKeys as any)[VISIBLE_KEY.SC]?.includes(visibleKeys.WELCOME)}
        />
        <Divider className={classes.dividerStyle} />
        {subCollectionData?.posts?.map((data: any, index: any) => {
          return (
            <div key={data?.topics?.heading?.snippet} className={classes.addingStyle}>
              <PostSnippetHeader
                ...
                isHideView={(invisibleKeys as any)[`${VISIBLE_KEY.SCP}_${index}`]?.includes(visibleKeys.HEADING)}
                icon={<PostIcon />}
              />
              <PostSnippet
                title="Description"
                ...
                isHideView={(invisibleKeys as any)[`${VISIBLE_KEY.SCP}_${index}`]?.includes(visibleKeys.DESCRIPTON)}
              />
        ...
        {subCollectionData?.subCollections?.map((data: any, index: any) => {
          return (
            <div key={data?.topics?.collectionName?.snippet} className={classes.addingStyle}>
              <div className={classes.namediv}>
                ...
                <PostSnippet
                  ...
                  isHideView={(invisibleKeys as any)[`${VISIBLE_KEY.SCSC}_${index}`]?.includes(visibleKeys.COLLECTIONNAME)}
                  titleOnly
                />
              </div>
              ...
              <PostSnippet
                ...
                isHideView={(invisibleKeys as any)[`${VISIBLE_KEY.SCSC}_${index}`]?.includes(visibleKeys.DESCRIPTON)}
              />
              <PostSnippet
                title="Welcome messsage"
                ...
                isHideView={(invisibleKeys as any)[`${VISIBLE_KEY.SCSC}_${index}`]?.includes(visibleKeys.WELCOME)}
              />

              <Divider className={classes.dividerStyle} />
              {data?.posts?.map((data1: any, index: any) => {
                return (
                  <div key={data1?.topics?.heading?.snippet} className={classes.addingStyle}>
                    <PostSnippetHeader
                      ...
                      isHideView={(invisibleKeys as any)[
              ...
              <PostSnippet
                ...
                isHideView={(invisibleKeys as any)[`${VISIBLE_KEY.SCSC}_${index}`]?.includes(visibleKeys.THANKYOU)}
              />
            </div>
          );
        })}
        ...
        <PostSnippet
          ...
          isHideView={(invisibleKeys as any)[VISIBLE_KEY.SC]?.includes(visibleKeys.THANKYOU)}
        />
      </div>
