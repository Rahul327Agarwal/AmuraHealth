...
export const knowledgeBaseSelectedCardAtom = atom('');

export default function KnowledgeBase(props: DistributionsListProps) {
  ...
  const [uniqueCollectionType, setUniqueCollectionType] = useState<UniqueCollectionType>('KBM');
  const [activeTab, setActiveTab] = useState('All');
  const [searchString, setSearchString] = useState('');
  const [cardData, setCardData] = useState<any>([]);
  const [selectedCardId, setSelectedCardId] = useAtom(knowledgeBaseSelectedCardAtom);
  ...
  console.log('inside knowledge base list');
  ...
  const addingNewPost = () => {
    ...
    setSelectedCardId('');
    numOfPanelsCanFit > 1 && dispatch(setDisabledaddBtnInList(true));
    dispatch(setDisabledaddBtnInLMS(false));
    dispatch(setDisabledaddBtnInPolls(false));
    dispatch(setDisabledaddBtnInQMT(false));
  ...
  return (
    <div className={classes.mainContainer} key={searchClose}>
      ...
      <div style={{ height: '100%', width: '100%' }} className={classes.nameChardWrapper}>
        <AutoSizer
          style={{
            height: '100%',
            width: '100%',
