...
export const lmsSelectedCardAtom = atom('');

export default function LMS(props: DistributionsListProps) {
  const { childEventTrigger, registerEvent, unRegisterEvent, panel, selectedClient } = props;
  const { classes } = useStyles(props);
  const dispatch = useDispatch();
  const { id: panelId } = useCurrentPanel();

  const disabledaddBtnInLMS = useSelector((state: IRootState) => state.AllPostData.disabledaddBtnInLMS);

  const [uniqueCollectionType, setUniqueCollectionType] = useState<UniqueCollectionType>('LMS');
  const [activeTab, setActiveTab] = useState('All');
  const [searchString, setSearchString] = useState('');
  const [cardData, setCardData] = useState<any>([]);
  const [selectedCardId, setSelectedCardId] = useAtom(lmsSelectedCardAtom);
  ...
  const addingNewPost = () => {
    ...
    setSelectedCardId('');
    numOfPanelsCanFit > 1 && dispatch(setDisabledaddBtnInLMS(true));
    dispatch(setDisabledaddBtnInList(false));
    dispatch(setDisabledaddBtnInPolls(false));
    dispatch(setDisabledaddBtnInQMT(false));
  ...
  return (
    <div className={classes.mainContainer} key={searchClose}>
      ...
      <Fab
        onClick={() => {
          if (!disabledaddBtnInLMS) {
            addingNewPost();
          } else {
          }
        }}
        className={disabledaddBtnInLMS ? classes.disabledButton : classes.addButton}
      >
        <PlusIcon />
      </Fab>
    </div>
