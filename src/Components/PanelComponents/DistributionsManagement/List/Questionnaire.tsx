...
export const questionnaireSelectedCardAtom = atom('');

export default function Questionnaire(props: DistributionsListProps) {
  const { childEventTrigger, registerEvent, unRegisterEvent, panel, selectedClient } = props;
  const { classes } = useStyles(props);
  const { id: panelId } = useCurrentPanel();
  const dispatch = useDispatch();

  const disabledaddBtnInQMT = useSelector((state: IRootState) => state.AllPostData.disabledaddBtnInQMT);
  const [uniqueCollectionType, setUniqueCollectionType] = useState<UniqueCollectionType>('QMT');
  const [activeTab, setActiveTab] = useState('All');
  const [searchString, setSearchString] = useState('');
  const [cardData, setCardData] = useState<any>([]);
  const [selectedCardId, setSelectedCardId] = useAtom(questionnaireSelectedCardAtom);
  ...
  const addingNewPost = () => {
    ...
    setSelectedCardId('');
    numOfPanelsCanFit > 1 && dispatch(setDisabledaddBtnInQMT(true));
    dispatch(setDisabledaddBtnInList(false));
    dispatch(setDisabledaddBtnInLMS(false));
    dispatch(setDisabledaddBtnInPolls(false));
  ...
  return (
    <div className={classes.mainContainer} key={searchClose}>
      ...
      <Fab
        onClick={() => {
          if (!disabledaddBtnInQMT) {
            addingNewPost();
          } else {
          }
        }}
        className={disabledaddBtnInQMT ? classes.disabledButton : classes.addButton}
      >
        <PlusIcon />
      </Fab>
    </div>
